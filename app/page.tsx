...
const STOPWORDS = new Set([
  ...
  "in","into","is","it","its","of","on","or","that","the","their","them","there",
  "they","this","to","was","were","will","with","your"
]);
...
function scoreDoc(queryTerms: string[], doc: Protocol) {
  ...
  for (const t of queryTerms) {
    ...
    score += 3 * titleTokens.filter(x => x === t).length;
  }
  return score;
}
...
export default function Page() {
  ...
  const visible = useMemo(() => {
    ...
    // No search terms -> show first 50 sorted by category then start_page
    if (terms.length === 0) {
      return rows
        .slice()
        .sort((a, b) => {
    ...
    // Score results for the entered terms
    const scored: { s: number; r: Protocol }[] = [];
    for (const r of rows) {
      const s = scoreDoc(terms, r);
      if (s > 0) {
        // ✅ Fix: arrays use push(), not append()
        scored.push({ s, r });
      }
    }
      ...
      <div className="mx-auto max-w-6xl">
        <header className="mb-8">
          ...
          <p className="text-sm text-slate-300/70 mt-1">
            Search titles and excerpts. Filter by category. Static export for Cloudflare Pages.
          </p>
        </header>
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  <div className="text-sm text-slate-400">
                    {p.category} • pages {p.start_page}-{p.end_page}
                  </div>
